...
std::vector<std::string> Config::findIncludePath(const std::string& name,
                                                 const std::vector<std::string>& dirs) {
  const auto match1{tryExpandPath(name, "")};
  if (!match1.empty()) {
    return match1;
  }
  if (const char* dir = std::getenv(Config::CONFIG_PATH_ENV)) {
...
void Config::resolveConfigIncludes(Json::Value& config, int depth) {
  const Json::Value includes{config["include"]};
  if (includes.isArray()) {
    for (const auto& include : includes) {
      spdlog::info("Including resource file: {}", include.asString());
      const auto matches{findIncludePath(include.asString())};
      if (!matches.empty()) {
    ...
    const auto matches{findIncludePath(includes.asString())};
    if (!matches.empty()) {
      for (const auto& match : matches) {
        setupConfig(config, match, depth + 1);
      }
...
bool isValidOutput(const Json::Value& config, const std::string& name,
  ...
  if (config["output"].isArray()) {
    for (auto const& output_conf : config["output"]) {
      if (output_conf.isString()) {
        const auto config_output{output_conf.asString()};
  ...
  if (config["output"].isString()) {
    const auto config_output{config["output"].asString()};
...
void Config::load(const std::string& config) {
  const auto file{config.empty() ? findConfigPath({"config", "config.jsonc"}) : config};
  if (!file) {
