...
auto fmt::formatter<waybar::Appearance>::format(waybar::Appearance c,
...
void waybar::Portal::refreshAppearance() {
  ...
  try {
    const auto container{
        Glib::VariantBase::cast_dynamic<Glib::VariantContainerBase>(response)};
    Glib::VariantBase modev{};
    container.get_child(modev, 0);
    const auto mode{
        Glib::VariantBase::cast_dynamic<
            Glib::Variant<Glib::Variant<Glib::Variant<uint32_t>>>>(modev)
            .get()
            .get()
            .get()};
    const auto newMode{Appearance(mode)};
    if (newMode == m_current_mode_) {
      return;
    }
    spdlog::info("Discovered appearance '{}'", newMode);
    m_current_mode_ = newMode;
    m_signal_appearance_changed_.emit(m_current_mode_);
...
waybar::Appearance waybar::Portal::getAppearance() { return m_current_mode_; };
...
void waybar::Portal::on_signal(const Glib::ustring &sender_name,
  ...
  const auto value{
      Glib::VariantBase::cast_dynamic<Glib::Variant<Glib::Variant<uint32_t>>>(
          valuev)
          .get()
          .get()};
  const auto newMode{Appearance(value)};
  if (newMode == m_current_mode_) {
    return;
  }
