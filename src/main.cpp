...
//vilu1214    waybar::SafeSignal<int> posix_signal_received;
//vilu1214    posix_signal_received.connect([&](int signum) { handleSignalMainThread(signum, reload); });

//vilu1214    std::thread signal_thread([&]() { catchSignals(posix_signal_received); });

    // Every `std::thread` must be joined or detached.
    // This thread should run forever, so detach it.
//vilu1214    signal_thread.detach();
    ...
    spdlog::error("{}", e.what());
    return 1;
  }/*vilu1214 catch (const Glib::Exception& e) {
